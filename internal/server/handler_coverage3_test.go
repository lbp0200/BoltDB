...
func TestExecuteCommand_DUMP_NonExistent_Coverage(t *testing.T) {
	...
	resp := handler.executeCommand("DUMP", [][]byte{[]byte("nonexistent")}, "127.0.0.1:12345")
...
func TestExecuteCommand_OBJECT_REFCOUNT_Coverage(t *testing.T) {
	...
	// Set a key
	handler.executeCommand("SET", [][]byte{[]byte("testkey"), []byte("testvalue")}, "127.0.0.1:12345")

	// OBJECT REFCOUNT
	resp := handler.executeCommand("OBJECT", [][]byte{[]byte("REFCOUNT"), []byte("testkey")}, "127.0.0.1:12345")
...
func TestExecuteCommand_CLIENT_NOEVICT_Error_Coverage(t *testing.T) {
	...
	resp := handler.executeCommand("CLIENT", [][]byte{[]byte("NOEVICT")}, "127.0.0.1:12345")
...
func TestExecuteCommand_CLIENT_TRACKING_Error_Coverage(t *testing.T) {
	...
	resp := handler.executeCommand("CLIENT", [][]byte{[]byte("TRACKING"), []byte("INVALID")}, "127.0.0.1:12345")
	err, ok := resp.(*proto.Error)
	assert.True(t, ok)
	assert.True(t, len(string(*err)) > 0)
}
...
func TestExecuteCommand_PFINFO_Coverage(t *testing.T) {
	...
	// PFINFO
	resp := handler.executeCommand("PFINFO", [][]byte{[]byte("myhyperloglog")}, "127.0.0.1:12345")
...
func TestExecuteCommand_SWAPDB_Coverage2(t *testing.T) {
	...
	handler.executeCommand("SET", [][]byte{[]byte("key0"), []byte("value0")}, "127.0.0.1:12345")

	// Select database 1 and set a key
	handler.executeCommand("SELECT", [][]byte{[]byte("1")}, "127.0.0.1:12345")
