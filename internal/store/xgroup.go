...
func (s *BotreonStore) XGroupCreate(key, group, startID string) error {
	...
	}, 30, encodePropagateCommand([]byte("XGROUP"), []byte("CREATE"), []byte(key), []byte(group), []byte(startID)))
}
...
func (s *BotreonStore) XGroupCreateConsumer(key, group, consumer string) (int64, error) {
	...
	err := s.retryUpdate(func(txn *badger.Txn) error {
		...
		return txn.Set(groupKey, data)
	}, 30, encodePropagateCommand([]byte("XGROUP"), []byte("CREATECONSUMER"), []byte(key), []byte(group), []byte(consumer)))
	return created, err
}
...
func (s *BotreonStore) XGroupDelConsumer(key, group, consumer string) (int64, error) {
	...
	err := s.retryUpdate(func(txn *badger.Txn) error {
		...
		return txn.Set(groupKey, data)
	}, 30, encodePropagateCommand([]byte("XGROUP"), []byte("DELCONSUMER"), []byte(key), []byte(group), []byte(consumer)))
	return removed, err
}
...
func (s *BotreonStore) XGroupDestroy(key, group string) error {
	...
	}, 30, encodePropagateCommand([]byte("XGROUP"), []byte("DESTROY"), []byte(key), []byte(group)))
}
...
func (s *BotreonStore) XGroupSetID(key, group, id string) error {
	...
	return s.retryUpdate(func(txn *badger.Txn) error {
		...
		return txn.Set(groupKey, data)
	}, 30, encodePropagateCommand([]byte("XGROUP"), []byte("SETID"), []byte(key), []byte(group), []byte(id)))
}
