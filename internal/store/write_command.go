...
// with handler methods to eliminate command-switch duplication.
func WriteCommand(s *BotreonStore, args [][]byte, ctx context.Context) error {
	if len(args) == 0 {
		return nil
	}

	// cmd 已在调用方解析，args[0] 是大写命令名
	cmd := string(args[0])
	...
	case "ZRANGESTORE":
		if len(args) >= 5 {
			...
			if byLex {
				...
				members = make([]ZSetMember, len(lexMembers))
				for i, m := range lexMembers {
					members[i] = ZSetMember{Member: m, Score: 0}
				}
			} else if byScore {
				...
				members = make([]ZSetMember, len(ptrMembers))
				for i, m := range ptrMembers {
					members[i] = ZSetMember{Member: m.Member, Score: m.Score}
				}
			}
	...
	case "COPY":
		if len(args) >= 3 {
			...
			case "zset":
				...
				zAdd := make([]ZSetMember, len(zMembers))
	...
	case "TS.MADD":
		...
		if len(args) >= 4 && (len(args)-1)%3 == 0 {
			for i := 1; i+2 < len(args); i += 3 {
				...
				if _, err := s.TSAdd(key, timestamp, value, TSAddOptions{}); err != nil {
	...
	case "XAUTOCLAIM":
		if len(args) >= 7 {
			...
			minIdleTime, _ := strconv.ParseInt(string(args[4]), 10, 64)
			start := string(args[5])
			opts := XAutoClaimOptions{Count: 100, JustID: false}
