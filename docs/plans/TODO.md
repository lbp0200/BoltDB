...
| 项 | 说明 |
|----|------|
| **BLPOP/BLMPOP 等阻塞命令不接受浮点 timeout** | 2026-08-25 双端差分发现：`BLPOP key 0.01` 在 redis-server 8.2.1 正常工作（Redis 6+ 接受浮点秒），BoltDB 报 `ERR timeout is not an integer or out of range`。redis-py/node-redis 默认传整数秒未暴露，但直接发协议的客户端会踩。修复方向：timeout 解析改 `ParseFloat` 后取整/上取整（与 Redis 的 `ms = seconds*1000 + 0.5` 对齐） |
