...
>    **log-key 增量流先行（分级 2/3 合并——零回传）**：master 侧 log 键前缀扫描增量发送
>    + 从侧 apply + ACK/PSYNC ts 语义 + lastAppliedTS——4 守卫重写 + dw A/B ≤1/15——大
>    设计——a4 §10 附7 定案锚）；4. ~~C5 开销归因~~ ✅（2026-09-03——同条件 A/B 定论：
