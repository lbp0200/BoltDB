...
- **工具就绪（2026-09-05 确认）**：`cmd/scale-data-filler` 已实现按槽分组 pipeline
  ...
  **测量机器 = 192.168.1.251（用户指示 2026-09-06——10.1.2.16 网络不可达不再尝试）**
- **机器可达性（2026-09-06 更新）**：10.1.2.16（GCP VM elex-gm0135）网络持续不可达
  ——**用户指示停止尝试——SSD 基线改用 192.168.1.251**——注：192.168.1.251
  此前记录为 HDD 环境——测量前需确认机器磁盘状态（SSD/HDD）与基线可比性——
  scale-data-filler 工具已就绪——前置三查完成即可启动
- **挂起（2026-09-06——用户决定暂缓）**：192.168.1.251 实测 **HDD 3.6T**
  （/dev/sda1 → /media/hdd4t——非 SSD）+ **无 boltDB 集群部署**（6337/6338/6339
  无监听——scale-data-filler 集群模式需先部署集群）——SSD 基线无法在现有机器
  严格复现（HDD 写路径行为不同——基线不可比）——**用户决定暂缓（§4 挂起）——
  时间投入 lost 偶发根因等方向**——恢复条件：未来获得 SSD 环境（部署集群 +
  scale-data-filler 全流程——工具与前置三查已就绪）
