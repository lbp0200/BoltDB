...
> 待续。探针已清理还原。
>
> **发散起点 A/B 定论（2026-09-03 03:25）——发散在应用期内出现**：主侧 ACK 分支
> 边界探针（每次 REPLCONF ACK 对从侧 offset 检查 backlog 边界）——dw_onset1：
> **10 PASS + 5 FAIL + 6 降级 + 5 停滞——全程 0 条非边界 ACK**（从侧 ACK 时刻
> offset 恒在主侧 backlog 边界上）——而降级（PSYNC 时刻）的 offset 非边界——
> **发散在 ACK 采样（~1s）间隙内的应用过程中出现**。叠加 0 逐命令失配（上）：
> 从侧应用期 offset 从边界变为非边界**而无任何逐命令长度失配**——悖论精化：
> 发散既非逐命令长度（0 失配）、亦非 ACK 可见（0 非边界）——只在应用-重连
...
>
> **应用段历史 A/B 定论（2026-09-03 03:55）——定位尝试穷尽**：从侧应用历史环
> （最近 32 条应用的 off/len/cmd——误判时转储）——dw_hist1：13 PASS + 2 FAIL——
> 转储的应用序列无异常命令（无乱码/mis-frame 形态）——与 0 逐命令失配 + 0 非
> 边界 ACK 一致：**从侧应用期完全自洽、无任何可见异常——发散在主侧流与其
> 发送的从侧字节之间的差异，从侧视角不可见**。§1c 定位尝试至此穷尽（代码级
> 七环节 + 运行期逐命令/ACK/应用历史三维度），完整修复依赖主侧发送字节与
> 从侧接收字节的直接比对（抓包级）——超出当前工具范围——**入册定论关闭**。
> 探针已清理。
>
> **完整修复设计（2026-09-03）**：设计空间分析与分阶段路线见
> [1c-complete-fix-design.md](1c-complete-fix-design.md)——层 A（触发消除：从侧角色
> 读闸 A1b——推荐）、层 B（误判稳健：双重确认 B1——推荐立即做）、层 C（丢失消除：
> 被发散悖论阻塞——需层 D 抓包级定位先行）、阶段 0 观测补全——门槛：dw A/B ≤1/15。
>
> **B1 双重确认 A/B 定论（2026-09-03 08:32）——未达标已回退**：停滞判定连续 2 个
> ACK 周期确认（stallSamples 样本链）+ LLen 读探针 A/B（-count=15 远端 -race）：
> **4 FAIL / 15（39,393 MISSING + ZADD 基数缺口——经典残留形态）——高于门槛 ≤1/15、
> 相对 30s 基线 2/15 无改善——单样本抖动带假设未获支持——B1 已完整回退**（代码还原
> 至 bf14430——工作区干净、守卫还原、探针清理——详见 1c-complete-fix-design.md §10）。
> 附带观测：lag=44 冻结 60s 无停滞 WARN——停滞检测测 idle 非应用进度——execute 卡写
