...
| allDispatchCommands 同步 + 防漂移测试 | 已完成 ✅（2026-08-23）：审计发现手写清单 `allDispatchCommands` 还有 **8 个命令**漏注册（GEORADIUSBYMEMBER/_RO、GEORADIUS_RO、SORT_RO、SPUBLISH、SSUBSCRIBE、SUBSTR、SUNSUBSCRIBE）——它们不在写集合所以启动检查抓不到，属静默漂移。补齐后 259=259 对齐；新增 `TestAllDispatchCommands_MatchesRegistry` 双向对比 commandRegistry，任何一侧新增命令都会在 CI 失败而非等到服务启动（7d7ce86） |
| arity 全量对齐 Redis 8.2（27 处） | 已完成 ✅（2026-08-23）：对全部 259 条 command_info 与 redis-server 8.2.1 `COMMAND INFO` 做 arity/firstKey/lastKey 差分，发现 **27 处 arity 错误**（arity 是 COMMAND 返回给客户端的参数校验依据）：可选参数命令偏小（EXPIREAT/PEXPIRE/PEXPIREAT 的 NX/XX/GT/LT、ZRANK/ZREVRANK 的 WITHSCORE、BGSAVE SCHEDULE、QUIT NOSAVE、GEORADIUS/GEOSEARCH/GEOSEARCHSTORE 最小参数、XADD/XREAD/XREADGROUP/XCLAIM/XAUTOCLAIM/XTRIM/ZRANGESTORE/ZDIFFSTORE/BLMOVE/MIGRATE/HSCAN/SSCAN/ZSCAN）、多算（ZINTER/ZUNION/XGROUP）、REPLCONF。修复后 **arity 0 差异**；剩余 163 处 firstKey/lastKey=0 是装饰性缺失（服务端集群路由不依赖，客户端影响小，记录待补）（004e411） |
| 复制传播闭环测试 | 已完成 ✅（2026-08-23）：复制对称性 checklist 步骤 4（attach slave → write → assert）的传播半侧补上——`TestGEORADIUSStore_PropagatesToBacklog`（STORE 以规范化 GEORADIUS 形式进 backlog，**纯读 GEORADIUS 不传播**、offset 不变）与 `TestBLMPOP_PropagatesToBacklog`（通用传播路径带 LEFT/COUNT 进 backlog）；重放半侧已有 `TestExecuteReplicatedCommand_BLMPOP/_GEORADIUSStore`，主从闭环完整（b86b0a2） |
| 命令元数据最终对齐（arity + firstKey/lastKey 双 0 差异） | 已完成 ✅（2026-08-23）：①写集合 114 命令 vs `WriteCommand` 重放全覆盖审计——无遗漏；②修正审计脚本正则 bug（`[]commandFlag` 的 `]` 截断导致误报 163 处 firstKey 缺失，实际大部分本就正确）后重跑：XDELEX arity -3→-5；阻塞命令 BLPOP/BRPOP/BZPOPMAX/MIN lastKey -1→**-2**（末尾是 timeout 非 key）；movablekeys 命令（BLMPOP/BZMPOP/LMPOP/ZMPOP/ZDIFF/ZINTER/ZINTERCARD/ZUNION/XREAD/XREADGROUP）与容器命令（DEBUG/OBJECT/XGROUP/XINFO）firstKey→0；STORE 命令 lastKey→1；MIGRATE/SPUBLISH/SSUBSCRIBE/SUNSUBSCRIBE 补官方位置。`TestCommandInfoKeyPositionsConsistency` 放宽 lastKey 负数语义。**259/259 与 redis-server 8.2.1 完全一致**（af71c2f） |
| node-redis 套件补新命令（110→116） | 已完成 ✅（2026-08-23）：node 侧补 BLMPOP COUNT 2（sendCommand 原始数组断言）、GEORADIUS STORE（ZCARD 验证目标 zset）、Sharded PubSub（SPUBLISH/SSUBSCRIBE/smessage 投递）。调试发现两个 node 客户端行为：`rPush(key, ...elements)` 变参只发第一个元素（须传数组）；BLMPOP transformReply 为 undefined（sendCommand 返回原始数组）。三套件全绿：py 245 / cli 86 / node 116（4f26088） |
| RESET/WAITAOF/BGREWRITEAOF 命令 | 已完成 ✅（2026-08-23）：补齐 3 个客户端可见缺口（259→262）：**RESET**（Redis 6.2+）丢弃 MULTI + 清 WATCH + 全退订（channel/pattern/shard），返回 "RESET"，**MULTI 内立即执行**（实测 Redis 8.2 确认是丢弃事务而非入队，加入 dispatch 不排队列表）；**WAITAOF**（Redis 7+）返回 [0,0]（无 AOF）；**BGREWRITEAOF** 报 "rewriting disabled"（与 Redis appendonly no 一致）。元数据取自 COMMAND INFO；防漂移测试立刻抓到 allDispatchCommands 漏注册并补齐（65a10a4） |
| 2026-08-24 自主巡检 9 项 | 已完成 ✅（2026-08-24，8 轮）：①依赖补丁 `badger 4.9.1→4.9.6` + `ristretto 2.2→2.4` + `compress 1.19.2` + `lz4 4.1.29` + `colorable 0.1.14→0.1.15` + `isatty 0.0.20→0.0.24` + `go-redis 9.17.2→9.22.0`（`go.uber.org/atomic`）；②`isUUIDFormat` 去 `i%37` 冗余并补 6 条边界用例；③`handler_core double-reader→single`（`TotalInputBytes` 恒 0）；④`invariants/state-machine/monitoring/failure-modes/snapshot` 5 文档 `handler.go` 失效引用全量更新至 `handler_core/dispatch/replication_handler`；⑤`scale-data-filler` 优先 `CLUSTER SHARDS` 回退 `SLOTS`（兼容 Redis 7+，`ClusterSlots` deprecation `nolint`）；⑥`BOLTDB_PASSWORD` 热路径缓存（`Handler.authPassword`，`main.go`+`setupTestHandler` 一次性 `SetAuthPassword`，`handler_dispatch:59`/`admin2:377` 去每命令 `os.Getenv`，`OW7` 隔离）；⑦`waitPauseWindow` `time.After` 循环泄漏改 `time.NewTimer` 显式 `Stop`；⑧`CLIENT KILL` 3 处 `mu.Lock→cancel→Unlock` 违反 `OW7/F2` 改 `Lock→capture→Unlock→cancel` 防死锁；⑨`store_coverage3_test` 等回归加固。`golangci-lint 0 issues` / `go vet 0` / `remote -race -short` 全绿 |
| keyspace_hits/misses 全量差分补齐（30 命令） | 已完成 ✅（2026-08-25）：对本机 redis-server 8.2.1 做 INFO stats 逐命令双端差分（含 hit/miss/WRONGTYPE/语法错误路径），补齐 30 个读命令的 lookup 计数——GETRANGE(SUBSTR)/GETDEL/GETEX/GETSET/HSTRLEN/HRANDFIELD/LPOS/LCS(逐key)/SINTER/SUNION/SDIFF/SINTERCARD/ZRANGEBYSCORE/ZREVRANGEBYSCORE/ZRANGEBYLEX/ZREVRANGEBYLEX/ZRANDMEMBER/ZMSCORE(整命令单次)/ZDIFF/ZUNION/ZINTER/ZINTERCARD/PFCOUNT(逐key)/TOUCH/SORT(_RO)/BITOP(仅源key)/COPY(源key)/XREAD/XREADGROUP/XCLAIM。**实证语义**：①WRONGTYPE 仍计 hit（lookup 先于类型检查）②语法错误不计（parse 先于 lookup）③XREAD/XREADGROUP 成功路径每 stream key **双计数**（Redis 8.2.1 实测行为，已复刻），错误路径单次 ④`SET..GET` 计数而普通 SET 不计 ⑤LCS/PFCOUNT/BITOP/TOUCH/set 系按 key 数计数。新增 `recordKeyspaceLookup(s)` helper（`slowlog.go`）。**顺带修复 NOGROUP 缺口**：store 层 XREADGROUP/XCLAIM/XAUTOCLAIM 组缺失原静默返回空，现返回 `*ErrNOGroup`（消息与 Redis 逐字对齐：XREADGROUP 带 `in XREADGROUP with GROUP option` 后缀，XCLAIM/XAUTOCLAIM 无后缀）。新增 `keyspace_stats_test.go` 回归测试。远程 `-race` store+server 双包全绿 + RESP shape + 复制回归 + 三方兼容套件 py 247 / cli 93 / node 122 全过 |
| 阻塞命令浮点 timeout + 负数永久阻塞修复 | 已完成 ✅（2026-08-25）：22 值边界矩阵双端差分（0/-1/-0.01/0.0001/1e-10/inf/nan/0x10/带空格/下划线/溢出整数等 × BLPOP/BLMPOP/BZPOPMIN/XREAD/GETEX/SETEX）。**修 3 类 bug**：①阻塞命令不接受浮点秒（Redis 6+ 接受）——新 `parseBlockTimeoutSeconds` 解析器完整复刻 getTimeoutFromObjectOrReply 实证语义（浮点接受、精确 0=永久、负数→`timeout is negative`、±Inf/溢出→`out of range`、NaN/空白/下划线→`not a float`、hex `0x10`=16s 兼容 strtold、<1ms 钳 1ms、>time.Duration 上限饱和防纳秒溢出）；②**负数 timeout 永久阻塞**（store 约定 `<=0=forever`，`BLPOP k -1` 直接挂死）+ XREAD `BLOCK -1` 与"未指定"哨兵值 -1 碰撞变成非阻塞读；③BRPOPLPUSH/BLMOVE 的 `0=单次尝试` 与其他阻塞命令的 `0=永久` 不一致——全部统一为毫秒参数 + `0=永久`（Redis 统一语义）。XREAD/XREADGROUP BLOCK 改严格整数校验（拒绝 `+5`/浮点/科学计数，与 Redis 一致）；SETEX/PSETEX 错误消息对齐为 `ERR value is not an integer or out of range`。更新 2 个 RESP shape 测试（原依赖 timeout 0 单次尝试）、BZMPOP 错误消息断言、store 测试秒→毫秒换算。新增 `blocking_timeout_test.go`。边界矩阵复测 **0 DIFF**，远程 `-race` 双包 + RESP shape + 三方兼容套件全过 |
