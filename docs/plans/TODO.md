...
| **P4 BZPOPMAX/MIN on string 不返回 WRONGTYPE** | `redis_cli_compat.sh` 402/406 行：对 string 类型 key 执行 BZPOPMAX/BZPOPMIN，期望返回 `WRONGTYPE`，实际返回 nil（空响应）——阻塞弹出未做类型检查 | 待修：BZPOP 对非 zset 类型应先返回 WRONGTYPE（2026-08-05 兼容套件 75/77，该 2 项失败与此有关，预存问题非本次改动引入） | ⚠️ 预存失败，待跟进 |
